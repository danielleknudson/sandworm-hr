...
var GraphView = Backbone.View.extend({
  ...
  plotLine: function (stocks) {
    var margin = {
      ...
      outerWidth = parseInt(d3.select('.graph').style('width')), //700,
      outerHeight = 300,
    ...
    var maxY = d3.max(lineData, function (stock) {
    ...
    x.domain(d3.extent(averageStocks, function (d) {
      return d.date;
    }));
    //set y-domain to min and max stock $ ranges
    y.domain([
      d3.min(lineData, function (stock) {
        return d3.min(stock, function (d) {
          return d.value;
        });
      }),
      d3.max(lineData, function (stock) {
        return d3.max(stock, function (d) {
          return d.value;
        });
      })
